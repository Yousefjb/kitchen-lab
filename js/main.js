  ...
  try {
    ...
    if (isNew) {
      if (item.kind === 'mishap') {
        ...
        setFace('happy', 2200);
        speakOut([PHRASES.newFind, item.name, item.desc]);
      }
      discover(res);
      toastDiscovery(res);
      ...
      FX.mini(c.x, c.y);
      setFace('happy', 1500);
      speakOut([item.name, PHRASES.again]);
